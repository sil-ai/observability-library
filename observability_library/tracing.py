...
This module is intentionally narrow: it builds a `TracerProvider`, wires
it to an OTLP/HTTP exporter, registers it as the global tracer provider,
and returns it. It does *not* call any `*Instrumentor().instrument()` —
that belongs to the application, since the choice of which libraries to
    ...
    *,
    ...
    Raises:
        TracingConfigurationError: If `require_tls` or `require_auth` is
            set and the corresponding check fails, if
            `extra_resource_attributes` overlaps the reserved keys, or
            if the `[tracing]` extra is not installed.
    """
    global _provider
...
def reset_tracing() -> None:
    ...
    worker thread terminates cleanly. The OTel API global still points
    to the (now shut-down) provider until the next `setup_tracer_provider`
    call replaces it — do not record spans in the gap.
